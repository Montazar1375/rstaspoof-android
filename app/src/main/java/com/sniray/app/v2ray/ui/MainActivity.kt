...
import com.sniray.app.databinding.ActivityMainBinding
import com.sniray.app.v2ray.enums.EConfigType
import com.sniray.app.v2ray.enums.PermissionType
import com.sniray.app.v2ray.extension.toast
import com.sniray.app.v2ray.extension.toastError
import com.sniray.app.v2ray.handler.AngConfigManager
import com.sniray.app.v2ray.handler.MmkvManager
import com.sniray.app.v2ray.handler.SettingsChangeManager
...
class MainActivity : HelperBaseActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private enum class MainTab { SERVERS, BYPASS, LOGS }

    private var currentTab = MainTab.SERVERS
    ...
    private val bypassEditLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        if (it.resultCode == RESULT_OK) {
            val id = it.data?.getLongExtra(RstaBypassEditActivity.EXTRA_CONFIG_ID, -1L) ?: -1L
            if (id >= 0) proxyViewModel.selectConfig(id)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            RstaProxyServiceState.runState.collect { state ->
                if (state is ProxyRunState.Error && mainViewModel.isRunning.value != true) {
    ...
    private fun setupBottomNav() {
        binding.bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.nav_bypass -> {
                    selectMainTab(MainTab.BYPASS)
                    true
                }
                R.id.nav_logs -> {
                    selectMainTab(MainTab.LOGS)
                    true
                }
                else -> false
            }
    ...
    private fun selectMainTab(tab: MainTab) {
        ...
        when (tab) {
            MainTab.SERVERS -> {
                binding.bottomNav.menu.findItem(R.id.nav_servers)?.isChecked = true
                binding.serversContent.isVisible = true
                binding.rstaFragmentHost.isVisible = false
                binding.tabGroup.isVisible = groupPagerAdapter.groups.size > 1
                binding.layoutTest.isVisible = true
                title = getString(R.string.title_server)
                updateBypassSubtitle()
            }
            MainTab.BYPASS -> {
                binding.bottomNav.menu.findItem(R.id.nav_bypass)?.isChecked = true
                binding.serversContent.isVisible = false
                binding.rstaFragmentHost.isVisible = true
                title = getString(R.string.tab_bypass)
                supportActionBar?.subtitle = null
                showRstaFragment(RstaBypassFragment::class.java)
            }
            MainTab.LOGS -> {
                binding.bottomNav.menu.findItem(R.id.nav_logs)?.isChecked = true
                binding.serversContent.isVisible = false
                binding.rstaFragmentHost.isVisible = true
                title = getString(R.string.tab_logs)
                supportActionBar?.subtitle = null
                proxyViewModel.refreshLogsFromServiceState()
                showRstaFragment(RstaLogsFragment::class.java)
            }
        }
    }

    private fun showRstaFragment(fragmentClass: Class<out Fragment>) {
        val tag = fragmentClass.simpleName
        val current = supportFragmentManager.findFragmentById(R.id.rsta_fragment_host)
        if (current != null && current.javaClass == fragmentClass) return
        supportFragmentManager.beginTransaction()
            .replace(R.id.rsta_fragment_host, fragmentClass, null, tag)
            .commit()
    }

    private fun updateBypassSubtitle() {
        ...
        lifecycleScope.launch {
            val bypass = withContext(Dispatchers.IO) {
                RstaBypassHolder.loadBypassConfig(this@MainActivity)
            }
            if (currentTab == MainTab.SERVERS) {
                supportActionBar?.subtitle = bypass?.name
            }
        }
    }
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        ...
        when (currentTab) {
            ...
            MainTab.BYPASS -> menuInflater.inflate(R.menu.menu_rsta_bypass, menu)
            MainTab.LOGS -> Unit
        }
    ...
    override fun onOptionsItemSelected(item: MenuItem) = when (item.itemId) {
        R.id.add_config -> {
            if (currentTab == MainTab.BYPASS) {
                bypassEditLauncher.launch(
                    Intent(this, RstaBypassEditActivity::class.java)
                        .putExtra(RstaBypassEditActivity.EXTRA_CONFIG_ID, 0L),
                )
            }
            true
        }
        ...
        R.id.real_ping_all_sni -> {
            toast(getString(R.string.connection_test_testing_count, mainViewModel.serversCache.count()))
            mainViewModel.testAllRealPingWithSni()
            true
        }
