...
object CoreConfigContextBuilder {
     ...
     */
    fun build(context: Context, guid: String, applySniBypass: Boolean? = null): CoreConfigContext? {
        ...
        val profileForOutbound = when (applySniBypass) {
            false -> RstaBypassConfigInjector.restoreOriginalEndpoint(guid, config)
            true -> RstaBypassHolder.effectiveProfile(context, guid, config)
            null -> RstaBypassHolder.effectiveProfile(context, guid, config)
        }

        // Step 1: Resolve the main outbound (always tag = TAG_PROXY).
        val primaryResolvedOutbound = resolveOutbound(
            AppConfig.TAG_PROXY,
            profileForOutbound,
        ) ?: run {
