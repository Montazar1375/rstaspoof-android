...
import com.sniray.app.v2ray.dto.ConfigResult
import com.sniray.app.v2ray.dto.CoreConfigContext
...
import com.sniray.app.v2ray.enums.BalancerStrategyType
...
import com.sniray.app.v2ray.util.PackageUidResolver
...
object CoreConfigManager {
     ...
     */
    fun getV2rayConfig4Speedtest(context: Context, guid: String, useSniChain: Boolean = false): ConfigResult {
        try {
            val configContext = CoreConfigContextBuilder.build(context, guid, applySniBypass = useSniChain)
                ?: return ConfigResult(status = false, guid = guid, errorMessage = "Failed to build config context")
            if (configContext.isCustom) {
                return buildV2rayCustomConfig(configContext)
            }
